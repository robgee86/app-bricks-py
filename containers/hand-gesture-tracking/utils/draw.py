...
def draw_predictions(
        ...
        for batch_idx in range(len(NHWC_int_numpy_frames)):
            ...
            if ld.size != 0 and len(irh) != 0:
                hands_metadata = draw_landmarks_gesture_label(
                    image, ld, irh, gestures, confidences, landmark_connections=landmark_connections
                )
...
def draw_landmarks_gesture_label(
        ...
        for ldm, irh, gest, conf in zip(
            landmarks, is_right_hand, gesture_labels, gesture_confidences, strict=False
        ):
