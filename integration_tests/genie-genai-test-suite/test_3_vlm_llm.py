...
def vlm_client():
    ...
    return ChatOpenAI(base_url=BASE_URL, model=VLM_MODEL_NAME, temperature=TEMPERATURE, timeout=TIMEOUT)


@pytest.fixture(scope="session")
def llm_client():
    """Create a shared LLM chat client for all tests"""
    return ChatOpenAI(base_url=BASE_URL, model=LLM_MODEL_NAME, temperature=TEMPERATURE, timeout=TIMEOUT)
...
class TestVLMStreaming:
    ...
    def test_vlm_streaming_per_image(self, vlm_client, llm_client, system_prompt, json_structure, image_path):
        ...
        # LLM enrichment step (streaming, max 30 words)
        print(f"\n--- Calling LLM to enrich VLM output (STREAMING) ---")
        llm_prompt = f"Based on this outfit analysis: {response_content}\n\nProvide a brief enriching comment or styling tip in maximum 30 words."
        llm_messages = [HumanMessage(content=llm_prompt)]

        llm_start = time.time() * 1000
        llm_response = ""
        llm_chunk_count = 0

        for chunk in llm_client.stream(llm_messages):
            if chunk.content:
                llm_response += chunk.content
                print(chunk.content, end="", flush=True)
                llm_chunk_count += 1

        llm_elapsed = (time.time() * 1000) - llm_start
        print(f"\n--- LLM enrichment completed in {llm_elapsed:.2f}ms ({llm_chunk_count} chunks) ---")

        # Assert LLM was called and returned content
        assert len(llm_response) > 0, "LLM enrichment should not be empty"
        print(f"LLM enrichment length: {len(llm_response)} characters")
...
class TestVLMInvoke:
    ...
    def test_vlm_invoke_per_image(self, vlm_client, llm_client, system_prompt, json_structure, image_path):
...
class TestVLMComparison:
    ...
    def test_streaming_vs_invoke_consistency(self, vlm_client, llm_client, system_prompt, json_structure, test_images):
        ...
        # LLM enrichment for streaming result
        print(f"\n[LLM ENRICHMENT - STREAMING MODE]")
        llm_prompt_streaming = (
            f"Based on this outfit analysis: {streaming_content}\n\nProvide a brief enriching comment or styling tip in maximum 30 words."
        )
        llm_messages_streaming = [HumanMessage(content=llm_prompt_streaming)]

        llm_streaming_start = time.time() * 1000
        llm_streaming_response = ""

        for chunk in llm_client.stream(llm_messages_streaming):
            if chunk.content:
                llm_streaming_response += chunk.content
                print(chunk.content, end="", flush=True)

        llm_streaming_time = (time.time() * 1000) - llm_streaming_start
        print(f"\nLLM Streaming: {len(llm_streaming_response)} chars in {llm_streaming_time:.2f}ms")
        ...
        # LLM enrichment for invoke result
        print(f"\n[LLM ENRICHMENT - INVOKE MODE]")
        llm_prompt_invoke = (
            f"Based on this outfit analysis: {invoke_content}\n\nProvide a brief enriching comment or styling tip in maximum 30 words."
        )
        llm_messages_invoke = [HumanMessage(content=llm_prompt_invoke)]

        llm_invoke_start = time.time() * 1000
        llm_invoke_response = ""

        for chunk in llm_client.stream(llm_messages_invoke):
            if chunk.content:
                llm_invoke_response += chunk.content
                print(chunk.content, end="", flush=True)

        llm_invoke_time = (time.time() * 1000) - llm_invoke_start
        print(f"\nLLM Invoke: {len(llm_invoke_response)} chars in {llm_invoke_time:.2f}ms")

        # Assertions
        assert len(streaming_content) > 0, "Streaming should return content"
        assert len(invoke_content) > 0, "Invoke should return content"
        assert len(llm_streaming_response) > 0, "LLM streaming enrichment should not be empty"
        assert len(llm_invoke_response) > 0, "LLM invoke enrichment should not be empty"

        print(f"\n--- Comparison Summary ---")
        print(f"Both VLM modes returned content: ✓")
        print(f"Both LLM enrichments returned content: ✓")
        print(f"VLM Streaming time: {streaming_time:.2f}ms")
        print(f"VLM Invoke time: {invoke_time:.2f}ms")
        print(f"LLM Streaming time: {llm_streaming_time:.2f}ms")
        print(f"LLM Invoke time: {llm_invoke_time:.2f}ms")
