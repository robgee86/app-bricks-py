...
- High-rate feeds (gyros, accelerometers — tens to hundreds of Hz) are bucketed to ~1s
  automatically and judged on level, spread and spectral pitch, so ordinary motion is
  learned as normal while sustained abnormal vibration — or the same vibration at a new
  frequency — fires (`stats.lane` says which)
